...
新发现的Hades恶意软件活动针对Python开发者环境，利用Bun工具包静默执行多层载荷，可窃取敏感数据、横向移动，甚至通过对抗性提示注入劫持AI安全分析系统。该攻击属于“高度复杂”的供应链攻击，在导入受感染包时立即运行。
...
AI红队测试已从2019年微软初创时的边缘领域发展为成熟学科，GPT-4的出现彻底改变了传统攻击模拟方法。红队专家指出，AI模型使原有测试工具失效，需重新设计针对AI系统的对抗性评估框架。
...
[AI worm prototype shows attackers don’t need Mythos to take over your network] 多伦多大学研究人员开发了一个由AI代理驱动的蠕虫原型，利用免费本地LLM和常见漏洞及配置错误，在模拟网络中成功自我复制。该实验表明，即使没有前沿模型，攻击者也能利用现有工具实现网络入侵。
...
微软和谷歌分别发布警告，指出网络犯罪分子正利用员工对AI工具的使用习惯重塑社会工程攻击，通过冒充ChatGPT、Copilot、DeepSeek等平台分发恶意软件、窃取凭证。攻击者利用全球对AI的兴趣作为诱饵，将传统钓鱼手段与AI工具结合。
...
Tenable CTO指出，AI模型将漏洞利用时间压缩至“负天数”，即攻击者在厂商发布补丁前就已武器化漏洞。建议从静态CVE评分转向AI驱动的暴露管理，以应对加速的攻击周期。
...
攻击者滥用ChatGPT的共享内容功能（chatgpt.com/s/域名）发起钓鱼活动，托管恶意软件。该手法利用用户对合法域名的信任，绕过传统安全检测。
...
OpenAI为ChatGPT推出锁定模式和活跃会话功能，旨在遏制提示注入数据窃取攻击。这些安全控制措施从AI技术角度增强了对话系统的防护能力，防止恶意用户通过精心设计的提示词窃取敏感信息。
...
Bugcrowd研究人员发布ExploitBench基准测试，显示Mythos模型在利用Google Chrome漏洞方面优于GPT-5.5。该基准测试独立评估了AI模型在漏洞利用方面的能力，揭示了不同模型在网络安全任务中的性能差异。
...
前沿AI模型同时加速了漏洞发现和利用，并大幅降低攻击成本，使传统威胁模型和手动流程不再可行。EXPOSURE 2026会议为网络安全专业人员提供了构建暴露管理程序的蓝图。
...
Palo Alto Networks PAN-OS的Captive Portal存在零日漏洞CVE-2026-0300（缓冲区溢出），可导致未认证远程代码执行。该漏洞严重等级高，已被积极利用，建议立即更新。
