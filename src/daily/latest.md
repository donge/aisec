...
名为Hades的恶意软件针对Python开发者环境，通过感染包导入时运行，利用Bun工具包静默执行多层载荷。该攻击能窃取敏感数据、横向移动，并通过对抗性提示注入劫持AI安全分析系统，属于高度复杂的供应链攻击。
...
AI红队测试自2019年微软成立以来已成熟，但GPT-4的出现打破了传统方法，使现有工具失效。这标志着AI安全测试需适应新型威胁，从传统漏洞发现转向应对AI特有的攻击向量。
...
[AI worm prototype shows attackers don’t need Mythos to take over your network：多伦多大学研究人员开发了一个由AI代理驱动的计算机蠕虫原型，成功在模拟网络中自我复制。该蠕虫使用本地运行的免费大语言模型，结合新旧漏洞和常见配置错误，展示了即使没有前沿模型也能实现网络入侵。]
...
微软和谷歌警告，网络犯罪分子正利用员工对AI工具的使用习惯重塑社会工程攻击，通过冒充ChatGPT、Copilot等平台分发恶意软件和窃取凭证。攻击者利用全球对AI的兴趣作为诱饵，扩大欺诈活动范围。
...
Tenable首席技术官指出，AI模型将漏洞利用时间压缩至“负天数”，即攻击者在补丁发布前就武器化漏洞。建议从静态CVE评分转向AI驱动的暴露管理，以应对加速的威胁。
...
攻击者滥用ChatGPT的共享内容功能，通过chatgpt.com/s/域名托管恶意软件，发起钓鱼攻击。该手法利用合法平台的可信度，增加用户中招风险。
...
OpenAI为ChatGPT推出锁定模式和活跃会话功能，旨在防止提示注入导致的数据窃取。这些安全控制措施从AI技术角度增强了对话系统的防护能力。
...
新基准测试ExploitBench显示，AI模型Mythos在利用Google Chrome漏洞方面表现优于GPT-5.5。该测试由Bugcrowd研究员发布，独立评估了AI模型在漏洞利用方面的能力。
...
前沿AI模型同时加速漏洞发现和利用，并降低攻击成本，使传统威胁模型和手动流程不再可行。EXPOSURE 2026会议为网络安全专业人员提供了构建暴露管理计划的蓝图。
...
CVE-2026-0300是PAN-OS用户ID认证门户中的缓冲区溢出漏洞，允许未经身份验证的远程代码执行。该零日漏洞已被利用，严重等级高，需立即关注。
